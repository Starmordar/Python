...
def print_users(users):
    ...
    for i, (lcol, rcol) in enumerate(zip(sorted_users[::2], sorted_users[1::2])):
        ...
        right_record = "{}{}".format("  ", get_user_record(users[rcol])) if rcol is not None else ""
        print(get_user_record(users[lcol]) + right_record)


def get_user_record(user, namewidth=17, usernamewidth=9, ellipsiswidth=0):
    initial = ""
    if user.middlename:
